...
- **🧵 全系统线程安全** — 所有的界面更新（日志、进度条、按钮状态）均已实现线程安全化，彻底杜绝 Windows 下的随机假死。
- **🏷️ 文件名智能命名** — **(v2.1 新特性)** 下载时自动抓取专利标题，文件保存为 `D1-CN115640636A-一种锂离子电池负极材料.pdf`，无需打开即可知晓内容。
- **🌐 全球主流国家全覆盖** — **(v2.1 新特性)** 增强正则解析，新增对 DE (德国), FR (法国), GB (英国), TW (台湾), AU (澳大利亚) 等国家专利号的自动识别。
